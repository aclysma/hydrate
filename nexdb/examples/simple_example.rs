...
fn main() {
    ...
    let (schema_cache, data) = {
        ...
        let mut edit_context = EditContext::new(schema_set.clone(), &undo_stack);
        //linker.finish();
        ...
        let vec3_obj = edit_context.new_object(default_location.clone(), &vec3_type);
        ...
        let aabb_type = edit_context
            ...
            .unwrap()
            .clone();
        let aabb_obj = edit_context.new_object(default_location.clone(), &aabb_type);
        ...
        let data = DataStorageJsonSingleFile::store_string(&edit_context, &object_ids);
    ...
    println!("--------------------- Restoring with linker");
    {
        ...
        let undo_stack = UndoStack::default();
        let mut edit_context2 = EditContext::new(schema_set.clone(), &undo_stack);

        DataStorageJsonSingleFile::load_string(&mut edit_context2, default_location.clone(), &data);
        let object_ids: Vec<_> = edit_context2.all_objects().copied().collect();
        let data2 = DataStorageJsonSingleFile::store_string(&edit_context2, &object_ids);
        println!("Data2: {}", data2);
    ...
    println!("--------------------- Restoring with schema cache");
    {
        ...
        let undo_stack = UndoStack::default();
        let mut edit_context3 = EditContext::new(schema_set.clone(), &undo_stack);

        DataStorageJsonSingleFile::load_string(&mut edit_context3, default_location, &data);
        let object_ids: Vec<_> = edit_context3.all_objects().copied().collect();
        let data3 = DataStorageJsonSingleFile::store_string(&edit_context3, &object_ids);
        println!("---------------------");
        println!("Data3: {}", data3);
