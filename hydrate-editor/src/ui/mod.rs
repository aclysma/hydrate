...
fn thumbnail_stack_size() -> egui::Vec2 {
    let size =
        THUMBNAIL_STACK_SIZE_PER_ELEMENT + THUMBNAIL_STACK_DEPTH as f32 * THUMBNAIL_STACK_SPACING;
    egui::vec2(size, size)
}
...
fn draw_thumbnail_stack(
    ...
    for i in (0..assets_to_draw.len()).rev() {
        ...
        let child_rect = egui::Rect {
            ...
            max: thumbnail_space.min
                + egui::vec2(
                    element_size + spacing * i_f32,
                    element_size + spacing * i_f32,
                ),
        };
        ...
        child_ui.add(
            egui::Image::new(
                thumbnail_image_loader
                    .thumbnail_uri_for_asset(editor_model.root_edit_context(), assets_to_draw[i]),
            )
            .max_size(egui::vec2(element_size, element_size)),
        );
    }
}
