...
pub fn schema_record_selector(ui: &mut egui::Ui, id: impl Hash, schema_name: &mut String, schema_set: &SchemaSet) -> egui::InnerResponse<Option<SchemaRecord>> {
    ...
    let search: Vec<_> = schema_set.schemas().values().filter(|x| x.try_as_record().is_some()).map(|x| x.name()).collect();
    let response = ui.add(egui_autocomplete::AutoCompleteTextEdit::new(schema_name, search));
    let fingerprint = schema_set.try_find_named_type(schema_name).map(|x| x.try_as_record()).flatten().cloned();
