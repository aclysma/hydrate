...
fn end_context_behavior_for_drag_value(ctx: InspectorContext, response: egui::Response, new_value: Value) -> Option<(Value, EndContextBehavior)> {
...
fn end_context_behavior_for_text_field(ctx: InspectorContext, response: egui::Response, new_value: Value) -> Option<(Value, EndContextBehavior)> {
    let behavior = if response.lost_focus() || response.drag_released() {
        EndContextBehavior::Finish
    } else {
        EndContextBehavior::AllowResume
...
fn end_context_behavior_for_checkbox(ctx: InspectorContext, response: egui::Response, new_value: Value) -> Option<(Value, EndContextBehavior)> {
            ...
            Schema::String => {
                simple_value_property(
                    ...
                    |ui, ctx| {
                        ...
                        if response.changed() {
                            end_context_behavior_for_text_field(ctx, response, Value::String(Arc::new(value)))
                        } else {
                            None
                        }
                    }
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if old_symbol_name != selected_symbol_name {
                                ...
                                ctx.action_sender.queue_edit("property_editor", vec![asset_id], move |edit_context| {
                                    edit_context.set_property_override(asset_id, captured_property_path, Some(new_value))?;
                                    Ok(EndContextBehavior::Finish)
                                });
                            }
...
fn show_property_context_menu(ctx: InspectorContext, response: Response) -> Response {
    let asset_id = ctx.asset_id;
    response.context_menu(|ui| {
        let has_override = ctx.editor_model.root_edit_context().has_property_override(asset_id, ctx.property_path).unwrap();
        if ui.add_enabled(has_override && !ctx.read_only, egui::Button::new("Clear Override")).clicked() {
            let captured_property_path = ctx.property_path.to_string();
            ctx.action_sender.queue_edit("property_editor", vec![asset_id], move |edit_context| {
        ...
        let has_prototype = ctx.editor_model.root_edit_context().asset_prototype(asset_id).is_some();
        if ui.add_enabled(has_prototype && !ctx.read_only, egui::Button::new("Apply Override")).clicked() {
            let captured_property_path = ctx.property_path.to_string();
            ctx.action_sender.queue_edit("property_editor", vec![asset_id], move |edit_context| {
                edit_context.apply_property_override_to_prototype(asset_id, captured_property_path)?;
                Ok(EndContextBehavior::Finish)
            });
            ui.close_menu();
        }
