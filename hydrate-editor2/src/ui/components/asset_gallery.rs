...
fn draw_asset_gallery_tile(
    ...
    crate::ui::drag_drop::drag_source(
        ui,
        egui::Id::new(asset_info.id),
        DragDropPayload::AssetReference(asset_info.id),
        |ui| {
            let mut is_on = false;
            ...
            let asset_name_as_string = asset_info
                .name
                .as_string()
                .cloned()
                .unwrap_or_else(|| asset_info.id.to_string());
            ...
            let response = response.context_menu(move |ui| {
                ...
                if ui
                    .add_enabled(
                        !is_generated,
                        egui::Button::new(format!("Delete {}", &asset_name_as_string)),
                    )
                    .clicked()
                {
